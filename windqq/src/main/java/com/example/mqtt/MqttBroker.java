...
public class MqttBroker implements MqttCallback {
    ...
    public void messageArrived(String topic, MqttMessage message) throws Exception {
        try {
            ...
            double lat = msgJson.getDouble("lat");
            double lng = msgJson.getDouble("lng");
            String device_code = msgJson.getString("device_code");
            String device_name = msgJson.getString("device_name");

            DaoLocation daoLocation = new DaoLocation(null, device_name, device_code, lat, lng);
            List<DaoLocation> hased = VoiceDbUtil.getInstance().isHased(device_code);
            if (hased==null){
                boolean b = VoiceDbUtil.getInstance().insertLocation(daoLocation);
                if (b) {
//                    Log.e("tag", "添加位置信息成功");
                } else {
                    Log.e("tag", "添加位置信息失败");
                ...
                hased.get(0).setUser(device_name);
                hased.get(0).setUserId(device_code);
                hased.get(0).setLat(lat);
                hased.get(0).setLng(lng);
                boolean b = VoiceDbUtil.getInstance().updateLocation(hased);
                if (b){
//                Log.e("tag","内容修改了");
                }else {
                    Log.e("tag","失败");

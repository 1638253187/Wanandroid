package com.example.windqq.view.emoji;
...
public class EmojiDao {
    ...
    public static String CopySqliteFileFromRawToDatabases(String SqliteFileName) throws IOException {
        ...
        //复制的话这里需要换成自己项目的包名
        File dir = new File("data/data/" + "com.zkzj.rtmp_terminal" + "/databases");
        ...
        if (!file.exists()) {
            try {
                file.createNewFile();
                inputStream = QQApp.getApp().getClass().getClassLoader().getResourceAsStream("assets/" + SqliteFileName);
